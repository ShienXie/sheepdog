...
static void client_handler(int fd, int events, void *data)
{
	...
	if (events & EPOLLIN) {
		...
		/*switch(split%wq_net_split) {
		
		case 0:
			queue_work(sys->net0_wqueue, &ci->rx_work);
			break;
		case 1:
			queue_work(sys->net1_wqueue, &ci->rx_work);
			break;
		case 2:
			queue_work(sys->net2_wqueue, &ci->rx_work);
			break;
		case 3:
	...
	if (events & EPOLLOUT) {
		...
		/*switch(split%4) {
		case 0:
			queue_work(sys->net0_wqueue, &ci->tx_work);
			break;
		case 1:
			queue_work(sys->net1_wqueue, &ci->tx_work);
			break;
		case 2:
			queue_work(sys->net2_wqueue, &ci->tx_work);
			break;
		case 3:
